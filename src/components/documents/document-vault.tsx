...
function ProfessionalShare({ documents, disabled, onCreated }: { documents: Map<string, BuyerDocument>; disabled: boolean; onCreated: (share: DocumentShare) => void }) {
  ...
  async function requestShare() {
    setPending(true);
    const documentIds = [...documents.values()].filter((doc) => isAcceptedAndCurrent(doc)).map((doc) => doc.id);
    const cleanProviderName = providerName.trim();
    const { data: shareId, error } = await createClient().rpc("request_document_share", { p_recipient_type: providerType, p_provider_name: cleanProviderName, p_document_ids: documentIds });
    setPending(false);
    if (error) return toast.error(error.message);
    if (!shareId) return toast.error("The handoff consent was recorded without a reference. Refresh and try again.");
    onCreated({ id: String(shareId), recipient_type: providerType, provider_name: cleanProviderName, document_ids: documentIds, status: "requested", consented_at: new Date().toISOString(), revoked_at: null });
    toast.success("Consent recorded. The pilot team will verify the handoff details.");
    setOpen(false);
    setProviderName("");
  }
...
function ShareHistory({ shares, onRevoked }: { shares: DocumentShare[]; onRevoked: (id: string) => void }) {
  ...
  async function revoke(id: string) { setPendingId(id); const { error } = await createClient().rpc("revoke_document_share", { p_share_id: id }); setPendingId(null); if (error) toast.error(error.message); else { onRevoked(id); toast.success("Handoff consent revoked."); } }
