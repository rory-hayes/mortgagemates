...
import { FileCheck2Icon, LayoutDashboardIcon, MenuIcon, NotebookTabsIcon, ShieldCheckIcon, UserRoundIcon } from "lucide-react";
    ...
    <header className="border-b bg-card">
      <div className="content-grid flex h-20 items-center justify-between gap-6">
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Member navigation">
          {portalLinks.slice(0, 4).map((item) => <Link key={item.href} href={preview ? `${prefix}${item.label.toLowerCase()}` : item.href} className={cn(buttonVariants({ variant: "ghost", size: "sm" }))}><item.icon data-icon="inline-start" />{item.label}</Link>)}
        </nav>
        <div className="flex items-center gap-3">
          ...
          <Sheet>
            ...
            <SheetContent side="right"><SheetHeader><SheetTitle>Member portal</SheetTitle><SheetDescription>Profile, documents, matching, and preparation.</SheetDescription></SheetHeader><nav className="flex flex-col gap-2 px-4">{portalLinks.map((item) => <Link key={item.href} href={preview ? `${prefix}${item.label.toLowerCase()}` : item.href} className={cn(buttonVariants({ variant: "ghost" }), "justify-start")}><item.icon data-icon="inline-start" />{item.label}</Link>)}</nav></SheetContent>
          </Sheet>
        </div>
      </div>
    </header>
