...
export default async function DocumentsPage() {
  ...
  const [{ data: requirements }, { data: documents }, { data: matches }] = await Promise.all([
  ...
  return <DocumentVault userId={user.id} requirements={(requirements ?? []) as Requirement[]} initialDocuments={(documents ?? []) as BuyerDocument[]} pairReadiness={(readiness ?? []) as never[]} />;
}
